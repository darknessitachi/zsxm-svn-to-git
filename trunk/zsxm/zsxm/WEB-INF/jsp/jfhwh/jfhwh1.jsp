...
.fsg_nr{ position:absolute; display:none;top:25px; right:-3px; background:#fff; border:1px solid #98B1C8; }			
				...
				<table width="100%" cellpadding="0" cellspacing="0" id="conTable" >
							...
							<tr>
								    ...
								    <td class="center" >
								    	...
								    	<input type="button" class="but" value="填   报" id="xgBtn" onclick="xgJfh()">
								    	<b><a target=_blank href="jfhsb!doExportExcelJfhwh.do?treekey=<s:property value="treekey"/>"><font color="red">导出金凤凰经费资助表</font></a></b>
									</td>
	...
	var colsOption = [ 
	{
		...
		renderer:function (v,r){
			var ss = "";
			if(r.DC != null && r.DC != ''){
				ss ="<font color=blue>已填报</font>";
			}else{
				ss = "<font color=red>未填报</font>";
			}
			return ss;
		}
	}, {
	...
	var gridOption = {
		...
		remoteSort : true,
		remoteFilter : true,
		loadURL : APP_PATH + '/jfhwh!getShzfList.do?treekey='+$F('treekey'),
		exportURL : APP_PATH + '/jfhwh!exportShzfList.do?treekey='+$F('treekey'),
		exportFileName : "企业信息",
		autoLoad : true
	};
	...
	function datash(s,d,r){
		...
		parent.openWin("jfhsb!preDatash1.do?sbid="+s+"&dwid="+d+"&ryid="+r,w,h);
	}
	
	function changeW(v){
		if(v == 'XM'){
			$('sname').innerHTML = '<input type="text" style="width:100px" name="name" value=""/>';
		}else if(v == 'BZ'){
	...
	function xgJfh(){
		...
		parent.openWin("jfhwh!preDataxg1.do?sbid="+mygrid.getSelectedRecord().SBID+"&dwid="+mygrid.getSelectedRecord().DWID+"&ryid="+mygrid.getSelectedRecord().RYID+"&treekey="+$F('treekey'),w,h);
	}
	...
	function loadTree(){
		if($("xtdlb").loading != 1){
			...
			tree.loadJSON("jfhwh!getShzfDwTree.do",function(){$('xtdlbloadimage').style.display="none";});
			$('xtdlb').loading = 1;			
		}
	}
	function setV(id){
