	...
	<div class="xwincontent" id="xwincontent" >
		...
		<table align="center" cellpadding="0" cellspacing="0" id="tab2">
      		<tr>
				...
				<td>
					<s:textfield  name="zsxm.dflxr" cssStyle="width:130;"></s:textfield>
				</td>
				
				<td class="tdleft">电话</td>
				<td>
					<s:textfield  name="zsxm.dftel" cssStyle="width:130;"></s:textfield>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<s:textfield  name="zsxm.dfphone" cssStyle="width:130;"></s:textfield>
				</td>
				<td class="tdleft">邮箱</td>
				<td>
					<s:textfield  name="zsxm.dfemail" cssStyle="width:130;"></s:textfield>
				</td>
			</tr>
			<tr>	
...
<script type="text/javascript" src="<c:url value="/js/rc_init.js"/>" ></script>	
	<script type="text/javascript">
		...
		function doSave(type){
			if($('zsxm.dflxr').value.trim() == ''){
				alert('姓名不能为空，请填写！');
				return false;
			}
			
			if(type ==1){
				var ajax = new AppAjax("zsxm!doLxrI.do",function(data){save_Back(data,type)}).submitForm("czrcForm");
			}else if(type == 2){
				var ajax = new AppAjax("zsxm!doLxrI.do",function(data){save_Back(data,type)}).submitForm("czrcForm");
			}else if(type == 3){
				var ajax = new AppAjax("zsxm!doLxrU.do",function(data){save_Back(data,type)}).submitForm("czrcForm");
			}
		}
