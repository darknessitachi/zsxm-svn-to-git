			...
			<div id="disTree">
				...
				<table width="100%" cellpadding="0" cellspacing="0" id="conTable" >
						...
						<tr>
							    ...
							    <td class="center" >
							    	<INPUT id="queryfs" class="selectBut2"  title="项目标引筛选" value="项目标引筛选" type=button onclick="O_D('queryfs','flwhsel','bottom');loadSelFlTree();">
							    	<s:hidden name="selbydm"></s:hidden>
									<input type="button" class="but" value="选择项目" id="queryBtn" onclick="selrcxx()">
							    	<input type="button" class="but" value="删除项目" id="b_sendmessage" onclick="delrcxx()">
...
<script>
	...
	function loadTree(){
		...
		tree.loadJSON("zsxmby!getXmbywhTree.do",function(){});
	}
	...
	function loadSelFlTree(){
		if($("treesel").loading != 1){
			...
			seltree.loadJSON("zsxmby!getXmbywhTree.do",function(){$('flwhselload').style.display="none";});
			$('treesel').loading = 1;		
		}
	}
	...
	<s:iterator id="col" value="gcols">
			{
				...
				<s:if test="gscript==1">
					,renderer:function (v,r){return '<a href="javascript:;" title="点击查看详细信息" onclick="querymx('+r.XMID+')">'+v+'</a>';}
				</s:if>
			},
		</s:iterator>
	...
	var gridOption = {
		...
		remoteSort : true,
		remoteFilter : true,
		loadURL : APP_PATH + '/zsxmby!getZsxmSelected.do',
		exportURL : APP_PATH + '/zsxmby!exportList.do',
		exportFileName : "列表",
		autoLoad : true
	};
	...
	function addfl(type){
		...
		if(treekey == 'root')
			{dm = '';}
		openWin("zsxmby!preAddXmby.do?flmap.type="+type+"&flmap.bydm="+dm+"&option=1","350","220");
	}
	
	
	function repfl(){
		if(treekey != "" && treekey!="root"){
			openWin("zsxmby!preRepXmby.do?bydm="+treekey+"&option=3","350","220");
		}else{
			alert("请选择需修改的数据！");
		}
	}
	...
	function delfl(){
		if(treekey != "" && treekey!="root"){
			if(window.confirm("您确定要删除当前数据！")){
				var ajax = new AppAjax("zsxmby!doDelXmby.do?bydm="+treekey,save_Back).submit();
			}
		}else{
			alert("请选择需删除的数据！");
		}
	...
	function delrcxx(){
		...
		var r = new Array();
		for(var i=0;i<len;i++){
			r.push(mygrid.getSelectedRecords()[i].XMID);
		}
		if(window.confirm("请确定需要删除《"+len+"》条记录!")){
			var ajax = new AppAjax("zsxmby!doDelSelectedXmmx.do?xmids="+r+"&bydm="+treekey,save_Back_mx).submit();
		}
	}
	...
	function selrcxx(){
		if(treekey != "" && treekey!="root"){
			openWin("zsxmby!getZsxmWaitSelect.do?bydm="+treekey,"750","500");
		}else{
			alert("请选择相应的项目标引");
		}
	}
