...
<html>
	...
	<body>
	<s:component template="xtitle" theme="app" value='园区服务'></s:component>
	<s:form name="czrcForm" action='rcda!preYqfwI.do' method="post" >
	...
	<div class="xwincontent" id="xwincontent" >
		...
		<table align="center" cellpadding="0" cellspacing="0" id="tab2">
      		<tr>
				...
				<td>
					<input type="text" class="Wdate" name="zsdw.rq" id="date" style="text-align:left;width:130" value="<s:property value="zsdw.rq"/>" onfocus="new WdatePicker(this,'%Y-%M-%D')" MINDATE="1960-01-01" readonly/>
				</td>
				
				<td class="tdleft">园区服务人</td>
				<td>
					<s:select name="zsdw.yqfwr" list="xtusers" cssStyle="width:130" listKey="userid" listValue="cnname" headerKey="" headerValue="--请选择--" />
				</td>
			</tr>
			
			<tr>
				<td class="tdleft">单位接待人</td>
				<td>
					<s:textfield  name="zsdw.dwjdr" cssStyle="width:130;"></s:textfield>
				</td>
				<td class="tdleft">服务内容</td>
				<td>
					<s:textfield  name="zsdw.fwnr" cssStyle="width:130;"></s:textfield>
				</td>
			</tr>
			<tr>	
...
<script type="text/javascript" src="<c:url value="/js/rc_init.js"/>" ></script>	
	<script type="text/javascript">
		...
		function doSave(type){
			...
			if(type ==1){
				var ajax = new AppAjax("zsdw!doYqfwI.do",function(data){save_Back(data,type)}).submitForm("czrcForm");
			}else if(type == 2){
				var ajax = new AppAjax("zsdw!doYqfwI.do",function(data){save_Back(data,type)}).submitForm("czrcForm");
			}else if(type == 3){
				var ajax = new AppAjax("zsdw!doYqfwU.do",function(data){save_Back(data,type)}).submitForm("czrcForm");
			}
		}
