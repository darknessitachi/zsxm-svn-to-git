...
<html>
	...
	<body >
	    ...
	    <div id="xwincontent">
	    	<table cellspacing="0" class="fxtable"  style="margin-bottom:10px;">
	        	...
	        	<tr>
	        		<td style="text-align:center;background:#E7F2FC;font:bold 12px 'lucida Grande',Verdana;width:25%;">&nbsp</td>
	        		<td>
	        		<s:if test="jfhsb.bz==1">
	        			<input type="checkbox" value=1 name="jfhsb.bz" checked>
	        		</s:if>
	        		<s:elseif  test="jfhsb.bz==7">
	        			<input type="checkbox" value=1 name="jfhsb.bz" checked>
	        		</s:elseif>
	        		<s:else>
	        			<input type="checkbox" value=1 name="jfhsb.bz">
	        		</s:else>
	...
	<script type="text/javascript">
		function doSave(){
			if($('opttype').value==1){
				var ajax = new AppAjax("jfhsb!doAddJfhsb.do",function(data){save_Back(data)}).submitForm("czrcForm");
			}else{
				var ajax = new AppAjax("jfhsb!doRepJfhsb.do",function(data){save_Back(data)}).submitForm("czrcForm");
			}
		}
