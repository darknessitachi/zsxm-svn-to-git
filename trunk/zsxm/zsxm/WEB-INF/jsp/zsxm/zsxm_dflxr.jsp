...
<html>
	...
	<body>
	<s:form name="czrcForm" action='zsxm!preLxr.do' method="post" >
 ...
 <!--个人信息 start-->
	...
	<div class="butbar" id="butbar">
		<div class="left">	
			...
			<input type="button" class="button3"  value="设置第一联系人" onclick="setxx()"/>
		</div>
	</div>	
	...
	<script type="text/javascript">
		...
		function addxx(){
			parent.openWin("zsxm!preLxrI.do?xmid="+$('xmid').value+"&opttype=1&pname="+$('pname').value,"500","240");
		}
		...
		function repxx(){
			...
			parent.openWin("zsxm!preLxrU.do?xmid="+$('xmid').value+"&xh="+rcsel[0].v+"&opttype=3&pname="+$('pname').value,"500","240");
		}
		...
		function delxx(){
			...
			if(window.confirm("您确定要删除 "+rcsel.length+" 条信息?")){
				var ajax = new AppAjax("zsxm!doLxrD.do",sava_back).submitForm("czrcForm");
			}
		}
		...
		function setxx(){
			...
			var ajax = new AppAjax("zsxm!doSetLxrForDy.do?xmid="+$('xmid').value+"&xh="+rcsel[0].v,set_back).submit();
		}
