...
function  activeChange(x){
	if(oldx != x){
		 ...
		 if (x=="rcdali1")  
		 {
		 	...
		 	$("rcda_dis").src = "fzhtview!preFzht.do?htid="+document.all.htid.value+"&opttype="+document.all.opttype.value+"&pname="+self.name;
		 }
		 if (x=="rcdali2")  
		 {
		 	$("rcdali2").className="on";
		 	$("rcda_dis").src = "fzhtview!preFzsq.do?htid="+document.all.htid.value+"&opttype="+document.all.opttype.value+"&pname="+self.name;
		 }
		 /*
		 if (x=="rcdali3")  
		 {
