package com.netwander.core;

/**
 * 系统级常量配置设置
 */
public class Constants {

	public static final String USER_SESSION_KEY = "com.netwander.explib";
	
	//public static final String UPLOAD_PATH = getProperty("application.uploadFolder","/uploadfiles");

	public final static int DEFAULT_PAGE_SIZE = 50;

	/**
	 * 分页常量设置
	 */
	public final static String PAGE_START = "limit.start";

	public final static String PAGE_SIZE = "limit.length";

	public final static String PAGE_COUNT = "limit.count";
}
