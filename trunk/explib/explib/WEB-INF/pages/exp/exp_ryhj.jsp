...
<html>
	...
	<body>
	<s:form name="czrcForm" action='exp!preRyhj.do' method="post" >
	<s:hidden name="rcid"></s:hidden>
	<s:hidden name="pname"></s:hidden>
	<s:hidden name="opttype"></s:hidden>
	<s:hidden name="winid" id="winid"></s:hidden>
 <!--个人信息 start-->
	<div class="title"  onclick=""><h2>获奖情况&nbsp&nbsp&nbsp&nbsp&nbsp
	...
	<div class="footer" style="background:#efefef;">
		<input class="btn_submit1" type="button" value="下 一 步" onclick="javascript:parent.activeChange('rcda8','exp!preShjz.do');"/>
		<s:if test="opttype==3">
	...
	<script type="text/javascript">
		...
		function addxx(){
			...
			diag.MessageTitle = "新增荣誉获奖";
			diag.Message = "请填写个人荣誉获奖情况";
			diag.URL = "exp!preRyhjI.do?rcid="+$('rcid').value+"&opttype=1&pname="+$('pname').value+"&winid=xxjlinfo1";
			diag.show();
		}
		...
		function repxx(){
			...
			diag.Height = 140;
			diag.ShowMessageRow=true;
			diag.MessageTitle = "修改荣誉获奖";
			diag.Message = "请修改个人荣誉获奖情况";
			diag.URL = "exp!preRyhjU.do?rcid="+$('rcid').value+"&xh="+rcsel[0].v+"&opttype=3&pname="+$('pname').value+"&winid=xxjlinfo1";
			diag.show();
		}
		...
		function delxx(){
			...
			if(window.confirm("您确定要删除 "+rcsel.length+" 条人才信息?")){
				var ajax = new AppAjax("exp!doRyhjD.do",sava_back).submitForm("czrcForm");
			}
		}
