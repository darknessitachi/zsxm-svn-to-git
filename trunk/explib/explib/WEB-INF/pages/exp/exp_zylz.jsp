...
<html>
	...
	<body>
	<s:form name="czrcForm" action='exp!preZylz.do' method="post" >
	<s:hidden name="rcid"></s:hidden>
	<s:hidden name="pname"></s:hidden>
	<s:hidden name="opttype"></s:hidden>
	<s:hidden name="winid" id="winid"></s:hidden>
 <!--个人信息 start-->
	<div class="title"  onclick=""><h2> 主要论著论文&nbsp&nbsp&nbsp&nbsp&nbsp
			...
			<s:iterator value="qlist">
				<tr>
					<td><input type="checkbox" id="dmkey" name="dmkey" value='<s:property value="xh"/>'></td>
						...
						<td width="100px"><s:property value="cbrq"/></td>
				</tr>				
			</s:iterator>
		</table>		
	</div>
	<div class="footer" style="background:#efefef;">
		<input class="btn_submit1" type="button" value="下 一 步" onclick="javascript:parent.activeChange('rcda13','exp!preZscq.do');"/>
		<s:if test="opttype==3">
	...
	<script type="text/javascript">
		...
		function addxx(){
			...
			diag.MessageTitle = "新增主要论著";
			diag.Message = "请填写个人主要论著论文";
			diag.URL = "exp!preZylzI.do?rcid="+$('rcid').value+"&opttype=1&pname="+$('pname').value+"&winid=xxjlinfo1";
			diag.show();
		}
		...
		function repxx(){
			...
			diag.Height = 180;
			diag.ShowMessageRow=true;
			diag.MessageTitle = "修改主要论著";
			diag.Message = "请修改个人主要论著论文";
			diag.URL = "exp!preZylzU.do?rcid="+$('rcid').value+"&xh="+rcsel[0].v+"&opttype=3&pname="+$('pname').value+"&winid=xxjlinfo1";
			diag.show();
		}
		...
		function delxx(){
			...
			if(window.confirm("您确定要删除 "+rcsel.length+" 条专家信息?")){
				var ajax = new AppAjax("exp!doZylzD.do",sava_back).submitForm("czrcForm");
			}
		}
