...
dhtmlXTabBar.prototype._scrollTo = function (tab) {
	var that = this;
	var row = tab.parentNode;
	if (!row._rowScroller._init) {
		this._initScroller(row);
	}
	if (this._vMode) {
...
dhtmlXTabBar.prototype._scrollRight = function () {
	var that = this.parentNode.parentNode.parentNode.parentNode.tabbar;
	var row = this.parentNode.parentNode;
	if (row.tabCount - row.scrollIndex < 2) {
		return;
	}
	if (!row.scrollIndex) {
		row.scrollIndex = 0;
...
dhtmlXTabBar.prototype._onClickHandler = function (e) {
	...
	(e || event).cancelBubble = true;
	target = this.tabbar._getTabTarget(target);
	if (!target || !this.tabbar.tabsId[target.idd]) {
		return;
	}
...
dhtmlXTabBar.prototype.addTab = function (id, text, size, position, row) {
	...
	var tab = this._createTab(text, size, this._TabCloseButton, nss, id);
	tab.idd = id;
	this.tabsId[id] = tab;
...
dhtmlXTabBar.prototype.setTabActive = function (tabId, mode, hide) {
	...
	if (tab) {
		this._setTabActive(tab, (mode === false));
	}
};
dhtmlXTabBar.prototype._setTabActive = function (tab, mode) {
	...
	if ((!mode) && !this.callEvent("onSelect", [tab.idd, this._lastActive ? this._lastActive.idd : null])) {
		return false;
	}
...
dhtmlXTabBar.prototype._createTab = function (text, size, IsCloseButton, nss, id) {
	var tab = document.createElement("DIV");
	tab.className = "dhx_tab_element dhx_tab_element_inactive";
	var imgid = "close_img_" + this.pid + "_" + id;
	...
	switch (this._tbst) {
	  ...
	  case "win_text":
		if (this._vMode) {
			...
			if (IsCloseButton) {
				thml += "<img style=\"display:none;\" id=" + imgid + " src=\"" + (this._imgPath + this._TabCloseButtonSrc) + "\" style=\"cursor:pointer;position:absolute;right:2px;bottom:4px;\" onclick=\"\" />";
			}
			thml += "</div>";
		} else {
			...
			if (IsCloseButton) {
				thml += "<img style=\"display:none;\" id=" + imgid + "  src=\"" + (this._imgPath + this._TabCloseButtonSrc) + "\" style=\"cursor:pointer;position:absolute;right:0px;top:4px;\" onclick=\"\" />";
			}
		...
		if (!nss[10]) {
			...
			tab.style.backgroundColor = nss[10];
		}
		break;
	}
	tab.innerHTML = thml;
...
dhtmlXTabBar.prototype.clearAll = function () {
	...
	if (z) {
		this._conZone.style.backgroundColor = z;
	}
	...
	switch (this._hrfmode) {
	  case "iframe":
		if (!this._glframe) {
			...
			z.innerHTML = "<iframe name='tabframe' frameborder='0' width='100%' height='100%' src='" + this._imgPath + "blank.html'></iframe>";
	  ...
	  case "iframes-on-demand":
		...
		z.innerHTML = "<iframe  name='tabframe' frameborder='0' width='100%' height='100%' src='" + ((this._hrfmode == "iframes") ? href : (this._imgPath + "blank.html")) + "'></iframe>";
...
dhtmlXTabBar.prototype._deactivateTab = function () {
	...
	if (oss["id_" + this._lastActive.idd]) {
		oss = oss["id_" + this._lastActive.idd];
	}
	if (oss[10]) {
...
dhtmlXTabBar.prototype.enableAutoSize = function (autoWidth, autoHeight) {
	...
	if (this._awdj && this._ahdj) {
		this._conZone.style.overflow = "hidden";
	} else {
		this._conZone.style.overflow = "auto";
	}
};
dhtmlXTabBar.prototype.enableAutoReSize = function (mode) {
	...
	if (this._EARS) {
		...
		if (arguments.length == 1) {
			if ((this.entBox.style.width || "").indexOf("%") == -1) {
				this.entBox.style.width = "100%";
			}
			if ((this.entBox.style.height || "").indexOf("%") == -1) {
				this.entBox.style.height = "100%";
			}
		}
		var self = this;
...
dhtmlXTabBar.prototype.adjustSize = function () {
	...
	for (var id in this._content) {
		...
		var h = Math.max(box.scrollHeight, box.offsetHeight);
	...
	if (flag) {
		this.setSize(x || this._conZone.offsetWidth, y || this._conZone.offsetHeight, true);
	}
	if (this._EARS) {
		this.adjustOuterSize();
	}
