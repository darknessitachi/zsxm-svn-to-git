function atCalendarControl(){
  ...
  this.addCalendarPad=function(){
   document.write("<div id='divCalendarpad' style='position:absolute;top:0;right:0;width:255;height:167;display:none;'>");
  ...
  // calendar.calendarClose.onclick=function(){calendar.hide();}
   ...
   calendar.goToday.title="选择今天";
	var day = new Date();
	var month=(day.getMonth()+1+"").length==1?"0"+(day.getMonth()+1):(day.getMonth()+1);
	var date=(day.getDate()+"").length==1?"0"+day.getDate():day.getDate();
   calendar.goToday.onclick=function(){
   	calendar.sltDate=day.getFullYear()+"-"+month+"-"+date;
    //calendar.target.value=calendar.sltDate;
     calendar.head.innerText = calendar.sltDate;
     var theDate=calendar.sltDate.split("-");
	    	...
	    	for(j=0;j<7;j++){
		    	n++;
		     	if(n>theFirstDay && n<=theMonthLen){
		      		daycount=n-theFirstDay;
		      		calendar.insertBodyCell(theRow,j,daycount);
		     	}else{
  ...
  this.show=function(targetObject,defaultDate,sourceObject){
   ...
   if((document.body.offsetWidth-(offsetPos.x+calendar.source.offsetWidth-document.body.scrollLeft))>calendar.calendarPad.style.pixelWidth){
    ...
    var calLeft=offsetPos.x-80;//syf修改
   }
   //alert(offsetPos.x);
		...
		if(d.indexOf("-")==4)
			{
				...
				if(d.substring(d.indexOf("-")+1,d.length).indexOf("-")==2||d.substring(d.indexOf("-")+1,d.length).indexOf("-")==1)
					{
						var temp = d.substring(d.indexOf("-")+1,d.length);
						if(temp.substring(temp.indexOf("-")+1,temp.length).length==2||temp.substring(temp.indexOf("-")+1,temp.length).length==1)
							{return true;}
						else{
							alert("默认日期的格式不正确\n\n默认日期可接受格式为:'yyyy-mm-dd'");
							//return false;
